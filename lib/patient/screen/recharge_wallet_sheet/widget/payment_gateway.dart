...
class StripePayment {
  Future<void> makePayment(
    ...
    try {
      paymentIntent = await PatientApiService.instance.createPaymentIntent(
      ...
      await Stripe.instance
        ...
        paymentSheetParameters: SetupPaymentSheetParameters(
          customFlow: true,
          merchantDisplayName: user?.fullname ?? PS.current.unKnown,
        ...
        transactions: [
          {
            ...
            "item_list": {
              ...
              "shipping_address": {
                "recipient_name": user?.fullname ?? PS.current.unKnown,
                "line1": address,
                "line2": "",
                "city": city,
...
class FlutterwavePayment {
  void makePayment({
    ...
    final Customer customer = Customer(
        email: email,
        name: user?.fullname ?? PS.current.unKnown,
    ...
    flutterwave.charge().then((value) {
      if (value.success == true) {
        ...
        onError(PS.current.paymentFailed);
      }
    }).onError((error, stackTrace) {
      onError(PS.current.paymentFailed);
    });
