...
import 'package:doclink/patient/screen/confirm_booking_screen/widget/transaction_complete_sheet.dart';
import 'package:doclink/patient/screen/recharge_wallet_sheet/widget/payment_gateway.dart';
  ...
  void onContinueTap(int type) {
    ...
    if (settings?.paymentGateway == 1) {
      ...
      StripePayment().makePayment(
        ...
        onError: (error) {
          Get.back();
          CustomUi.infoSnackBar(PS.current.paymentFailed);
        },
        onSuccess: (value) {
          Get.back();
          apiCall(type,
      ...
      PaystackPayment().makePayment(
        ...
        onSuccess: (response) {
          if (jsonDecode(response)['status'] == true) {
            ...
            CustomUi.infoSnackBar(PS.current.paymentCancelled);
          }
        },
      );
      ...
      PaypalPayment().makePayment(
        ...
        onError: (error) {
          CustomUi.infoSnackBar(PS.current.somethingWentWrong);
        },
        onCancel: (param) {
          CustomUi.infoSnackBar(PS.current.paymentCancelled);
        },
