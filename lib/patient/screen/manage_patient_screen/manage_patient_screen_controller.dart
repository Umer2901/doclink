...
import 'package:doclink/patient/screen/manage_patient_screen/widget/add_patient_sheet.dart';
...
class ManagePatientScreenController extends GetxController {
  ...
  void fetchPatientApiCall() {
    isLoading = true;
    PatientApiService.instance.fetchPatient().then((value) {
      patients = value.data ?? [];
      isLoading = false;
  ...
  bool isValid() {
    ...
    if (imageFile == null && networkImage == null) {
      i++;
      CustomUi.snackBar(
          message: PS.current.pleaseAddYourPhoto, iconData: Icons.image_rounded);
    }
  ...
  void editPatient(
    ...
    selectGender = genders.firstWhere((element) {
      return element ==
          (patients?.gender == 1 ? PS.current.male : PS.current.female);
    });
  ...
  void deletePatientApiCall({int? patientID}) {
    CustomUi.loader();
    PatientApiService.instance.deletePatient(patientId: patientID).then((value) {
      Get.back();
      if (value.status == true) {
        CustomUi.snackBar(
