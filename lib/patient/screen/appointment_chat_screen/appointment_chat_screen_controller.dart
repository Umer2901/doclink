...
import 'package:doclink/patient/screen/video_call_screen/video_call_screen.dart';
    ...
    collection?.doc(time).set(
          AppointmentChat(
            ...
            senderUser: AppointmentUser(
              ...
              identity: '${FirebaseRes.pt}${appointmentData?.user?.identity}',
  ...
  onJoinMeeting(AppointmentChat data) {
    if (data.videoCall?.isStarted == true) {
      ...
      if (date1.isBefore(date2)) {
        ...
        CustomUi.snackBar(
          message: PS.current.pleaseWaitYourMeetingEtc,
          iconData: Icons.meeting_room_rounded,
        );
      }
    } else {
      CustomUi.snackBar(
        message: PS.current.meetingEnd,
