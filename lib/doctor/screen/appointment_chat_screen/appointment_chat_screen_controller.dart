...
import 'package:doclink/doctor/common/image_send_sheet.dart';
import 'package:doclink/doctor/common/video_upload_dialog.dart';
...
import 'package:doclink/doctor/screen/video_call_screen/video_call_screen.dart';
...
class AppointmentChatScreenController extends GetxController {
    ...
    await collection?.doc(time).set(
          AppointmentChat(
            ...
            senderUser: ChatUser(
              ...
              identity:
                  '${FirebaseRes.dr}${appointmentData?.doctor?.mobileNumber}',
              dob: '',
            ),
          ).toJson(),
        );
    if (appointmentData?.user?.isNotification == 1) {
      DoctorApiService().pushNotification(
  ...
  void onVideoTap() async {
    ...
    if (video != null) {
      ...
      if (sizeInMb <= 15) {
        CustomUi.loader();
        DoctorApiService.instance.uploadFileGivePath(File(video.path)).then((value) {
          videoUrl = value.path;
        });
  ...
  void onSendMediaTap(
    ...
    if (type == 0) {
      if (imageUrl == null) {
        await DoctorApiService.instance.uploadFileGivePath(File(image)).then((value) {
          imageUrl = value.path;
        });
      }
      Get.back();
